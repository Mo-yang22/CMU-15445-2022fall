...
TEST(LockManagerTest, UpgradeTest) {
  ...
  lock_mgr.LockTable(txn4, LockManager::LockMode::SHARED, oid);
  lock_mgr.LockTable(txn4, LockManager::LockMode::EXCLUSIVE, oid);
  lock_mgr.UnlockTable(txn4, oid);
  // txn_mgr.Commit(txn0);
  // txn_mgr.Begin(txn0);
  // CheckTableLockSizes(txn0, 0, 0, 0, 0, 0);
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  // assert(txn0->GetState() != TransactionState::COMMITTED);
