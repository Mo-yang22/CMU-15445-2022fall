...
LRUKReplacer::LRUKReplacer(size_t num_frames, size_t k) : replacer_size_(num_frames), k_(k),is_evictable_(num_frames,false) {}
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
    std::scoped_lock<std::mutex> lock(latch_);
    //不懂这个函数所说的其他情况是指啥意思,要我将is_evictable初始化为true?
    if(frame_id > static_cast<frame_id_t>(replacer_size_) ||(history_map_.find(frame_id) != history_map_.end()&&!is_evictable_[frame_id]) ){
        std::throw_with_nested("invalid frame_id");
        BUSTUB_ASSERT("cuo","wu");
    }
